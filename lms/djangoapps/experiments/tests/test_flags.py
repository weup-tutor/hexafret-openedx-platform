...
class ExperimentWaffleFlagTests(SharedModuleStoreTestCase):
    ...
    def setUp(self):
        ...
        self.flag = ExperimentWaffleFlag('experiments.test', __name__, num_buckets=2, experiment_id=0)  # pylint: disable=toggle-missing-annotation
    ...
    def test_forcing_bucket(self, active, expected_bucket):
        bucket_flag = CourseWaffleFlag('experiments.test.0', __name__)  # pylint: disable=toggle-missing-annotation
    ...
    def test_app_label_experiment_name(self):
        ...
        flag = ExperimentWaffleFlag("namespace.flag.name", __name__)  # pylint: disable=toggle-missing-annotation
...
class ExperimentWaffleFlagCourseAwarenessTest(SharedModuleStoreTestCase):
    ...
    """
    course_aware_flag = ExperimentWaffleFlag(  # pylint: disable=toggle-missing-annotation
        'exp.aware', __name__, num_buckets=20, use_course_aware_bucketing=True,
    )
    course_aware_subflag = CourseWaffleFlag('exp.aware.1', __name__)  # pylint: disable=toggle-missing-annotation

    course_unaware_flag = ExperimentWaffleFlag(  # pylint: disable=toggle-missing-annotation
        'exp.unaware', __name__, num_buckets=20, use_course_aware_bucketing=False,
    )
    course_unaware_subflag = CourseWaffleFlag('exp.unaware.1', __name__)  # pylint: disable=toggle-missing-annotation
