...
@contextmanager
def track_memory_usage(metric, course_id):  # pylint: disable=unused-argument
    """
    ...
    for memory_type, baseline_usage in zip(memory_types, baseline_usages):  # noqa: B905
        ...
        memory_used = total_usage - baseline_usage  # pylint: disable=unused-variable  # noqa: F841


def _generate_items_for_subtask(
    item_querysets,  # # pylint: disable=bad-option-value
    item_fields,
    total_num_items,
    items_per_task,
    total_num_subtasks,
    ...
    if num_items_queued != total_num_items:
        TASK_LOG.info("Number of items generated by chunking %s not equal to original total %s", num_items_queued, total_num_items)  # pylint: disable=line-too-long
...
class SubtaskStatus:
    ...
    def __init__(self, task_id, attempted=None, succeeded=0, failed=0, skipped=0, retried_nomax=0, retried_withmax=0, state=None):  # pylint: disable=line-too-long
...
def check_subtask_is_valid(entry_id, current_task_id, new_subtask_status):
    ...
    if current_task_id not in subtask_status_info:
        format_str = "Unexpected task_id '{}': unable to find status for subtask of instructor task '{}': rejecting task {}"  # pylint: disable=line-too-long
        msg = format_str.format(current_task_id, entry, new_subtask_status)
        TASK_LOG.warning(msg)
        raise DuplicateTaskException(msg)
    ...
    if subtask_state in READY_STATES:
        format_str = "Unexpected task_id '{}': already completed - status {} for subtask of instructor task '{}': rejecting task {}"  # pylint: disable=line-too-long
    ...
    if subtask_state == RETRY:
        ...
        if new_retry_count < current_retry_count:
            format_str = "Unexpected task_id '{}': already retried - status {} for subtask of instructor task '{}': rejecting task {}"  # pylint: disable=line-too-long
            msg = format_str.format(current_task_id, subtask_status, entry, new_subtask_status)
            TASK_LOG.warning(msg)
            raise DuplicateTaskException(msg)
...
def update_subtask_status(entry_id, current_task_id, new_subtask_status, retry_count=0):
    ...
    try:
        ...
        if retry_count < MAX_DATABASE_LOCK_RETRIES:
            ...
            TASK_LOG.info("Failed to update status after %d retries for subtask %s of instructor task %d with status %s",  # pylint: disable=line-too-long
