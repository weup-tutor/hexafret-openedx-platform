...
def _generate_items_for_subtask(
    ...
    if num_items_queued != total_num_items:
        TASK_LOG.info("Number of items generated by chunking %s not equal to original total %s", num_items_queued, total_num_items)  # noqa: E501
...
class SubtaskStatus:
    ...
    def __init__(self, task_id, attempted=None, succeeded=0, failed=0, skipped=0, retried_nomax=0, retried_withmax=0, state=None):  # noqa: E501
...
def check_subtask_is_valid(entry_id, current_task_id, new_subtask_status):
    ...
    if current_task_id not in subtask_status_info:
        format_str = "Unexpected task_id '{}': unable to find status for subtask of instructor task '{}': rejecting task {}"  # noqa: E501
        msg = format_str.format(current_task_id, entry, new_subtask_status)
        TASK_LOG.warning(msg)
        raise DuplicateTaskException(msg)
    ...
    if subtask_state in READY_STATES:
        format_str = "Unexpected task_id '{}': already completed - status {} for subtask of instructor task '{}': rejecting task {}"  # noqa: E501
    ...
    if subtask_state == RETRY:
        ...
        if new_retry_count < current_retry_count:
            format_str = "Unexpected task_id '{}': already retried - status {} for subtask of instructor task '{}': rejecting task {}"  # noqa: E501
            msg = format_str.format(current_task_id, subtask_status, entry, new_subtask_status)
            TASK_LOG.warning(msg)
            raise DuplicateTaskException(msg)
...
def update_subtask_status(entry_id, current_task_id, new_subtask_status, retry_count=0):
    ...
    try:
        ...
        if retry_count < MAX_DATABASE_LOCK_RETRIES:
            ...
            TASK_LOG.info("Failed to update status after %d retries for subtask %s of instructor task %d with status %s",  # noqa: E501
