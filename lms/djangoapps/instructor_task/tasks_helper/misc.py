...
def cohort_students_and_upload(_xblock_instance_args, _entry_id, course_id, task_input, action_name):  # pylint: disable=too-many-statements
    ...
    with DefaultStorage().open(task_input['file_name']) as f:
        ...
        for row in reader:
            ...
            try:
                ...
                (user, previous_cohort, preassigned) = add_user_to_cohort(cohorts_status[cohort_name]['cohort'], username_or_email)  # pylint: disable=unused-variable  # noqa: E501
                if preassigned:
    ...
    output_header = ['Cohort Name', 'Exists', 'Learners Added', 'Learners Not Found', 'Invalid Email Addresses', 'Preassigned Learners']  # noqa: E501
    output_rows = [
        [
