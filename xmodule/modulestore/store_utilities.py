...
def rewrite_nonportable_content_links(source_course_id, dest_course_id, text):
    ...
    try:
        ...
        logging.warning("Error producing regex substitution %r for text = %r.\n\nError msg = %s", jump_to_link_base, text, str(exc))  # noqa: E501
    ...
    if source_course_id != dest_course_id:
        try:
            ...
            text = re.sub(_prefix_only_url_replace_regex(generic_courseware_link_base), portable_asset_link_subtitution, text)  # noqa: E501
        except Exception as exc:  # pylint: disable=broad-except
            logging.warning("Error producing regex substitution %r for text = %r.\n\nError msg = %s", source_course_id, text, str(exc))  # noqa: E501
