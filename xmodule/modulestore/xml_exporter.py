    ...
    for field_name, field in subtree.fields.items():
        if field.is_set_on(subtree):
            if isinstance(field, Reference):
                ...
                field.write_to(
                    subtree, {
                        key: ele.map_into_course(destination_course_key) for key, ele in field.read_from(subtree).items()  # noqa: E501
                    }
                )
...
def export_extra_content(export_fs, modulestore, source_course_key, dest_course_key, category_type, dirname, file_suffix=''):  # pylint: disable=missing-function-docstring  # noqa: E501
