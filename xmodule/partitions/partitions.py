...
class UserPartition(namedtuple("UserPartition", "id name description groups scheme parameters active")):
    ...
    def get_scheme(name):
        ...
        try:
            ...
            raise UserPartitionError(f"Unrecognized scheme '{name}'")  # lint-amnesty, pylint: disable=raise-missing-from  # noqa: B904, E501
        scheme.name = name
        return scheme
    ...
    def from_json(value):
        ...
        if getattr(scheme, 'read_only', False):
            raise ReadOnlyUserPartitionError(f"UserPartition dict {value} uses scheme {scheme_id} which is read only")
