...
def stylesheet(parser, token):  # pylint: disable=unused-argument
    ...
    try:
        ...
        raise template.TemplateSyntaxError(  # lint-amnesty, pylint: disable=raise-missing-from  # noqa: B904
            '%r requires exactly one argument: the name of a group in the PIPELINE["STYLESHEETS"] setting' %  # noqa: UP031
...
def javascript(parser, token):  # pylint: disable=unused-argument
    ...
    try:
        ...
        raise template.TemplateSyntaxError(  # lint-amnesty, pylint: disable=raise-missing-from  # noqa: B904
            '%r requires exactly one argument: the name of a group in the PIPELINE["JAVASCRIPT"] setting' %  # noqa: UP031
            token.split_contents()[0]
        )
